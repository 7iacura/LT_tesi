...
\begin{itemize}
 ...
 \item \dja{include "login.html"} è la chiamata per includere in quella sezione della barra di navigazione il file dedicato al login, nel quale in base allo stato del cliente (loggato o no) mostrerà il form di login oppure i dati personali
...
In ogni template HTML è necessario inserire la riga di codice \dja{load i18n} per permettere a Django di individuare le stringe da tradurre, inoltre esse dovranno essere più precisamente indicate. 

Una soluzione è il tag \texttt{trans} utilizzato ad esempio
\begin{footnotesize}
\begin{verbatim}
...
Un altra soluzione è l'utilizzo del tag \dja{blocktrans}, di seguito un esempio di utilizzo
\begin{footnotesize}
\begin{verbatim}
 {% blocktrans %} 
